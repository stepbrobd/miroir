...
func (Init) Run(p Params) error {
	...
	if _, err := os.Stat(gitDir); os.IsNotExist(err) {
		...
		if err := run(p.RunCtx, p.Path, p.Ctx.Env, out,
	...
	setRemote := func(rname, uri string) error {
		_ = runQuiet(p.RunCtx, p.Path, p.Ctx.Env, "remote", "remove", rname)
		return runQuiet(p.RunCtx, p.Path, p.Ctx.Env, "remote", "add", rname, uri)
	}
	...
	if err := run(p.RunCtx, p.Path, p.Ctx.Env, out, fetchArgs...); err != nil {
		return err
	}
